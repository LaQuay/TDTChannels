...
+added
"""
        self.assertEqual({6, 7}, validator.added_line_numbers(diff))
...
class EntryValidationTests(unittest.TestCase):
    def test_valid_entry_supports_multiple_labeled_streams(self):
        ...
        self.assertEqual([], validator.validate_entry(item))
    ...
    def test_rejects_bad_link_label_and_info_spacing(self):
        ...
        item = validator.dataclasses.replace(item, info="GEO, EMB")
        messages = [problem.message for problem in validator.validate_entry(item)]
    ...
    def test_changed_malformed_row_is_blocking(self):
        text = "\n".join(
            [
                ...
                "## Nacionales",
                "",
                validator.EXPECTED_HEADERS["TELEVISION.md"],
                "| - | - | - | - | - | - |",
                "| Broken | [m3u8](https://example.com/live.m3u8) | [web](https://example.com) | - | - |",
            ]
        )
        selected, problems = validator.changed_entries("TELEVISION.md", text, {7})
    ...
    def test_unchanged_legacy_row_is_not_format_validated(self):
        ...
        selected, problems = validator.changed_entries("TELEVISION.md", text, {4})
    ...
    def test_changed_non_pipe_text_inside_table_is_blocking(self):
        ...
        _, problems = validator.changed_entries("TELEVISION.md", text, {4})
    ...
    def test_legacy_problem_is_grandfathered_while_field_is_unchanged(self):
        ...
        changed = validator.dataclasses.replace(legacy, web="[web](https://example.com/new)")
        problems = validator.validate_entry(changed)
        self.assertEqual([], validator.new_validation_problems([legacy], [changed], problems))

    def test_new_problem_on_changed_field_is_blocking(self):
        base = entry()
        changed = validator.dataclasses.replace(base, stream="[video](https://media.example/live)")
        problems = validator.validate_entry(changed)
        self.assertTrue(validator.new_validation_problems([base], [changed], problems))
...
class DuplicateTests(unittest.TestCase):
    def test_existing_duplicate_is_grandfathered(self):
        ...
        self.assertEqual([], validator.new_duplicate_problems(base, head))

    def test_increased_duplicate_count_is_blocking(self):
        base = [entry("Same")]
        head = [entry("Same"), entry("Same")]
        messages = [problem.message for problem in validator.new_duplicate_problems(base, head)]
    ...
    def test_names_are_scoped_per_catalog(self):
        ...
        radio = validator.dataclasses.replace(
            tv,
            catalog="RADIO.md",
            stream="[mp3](https://radio.example/live.mp3)",
        )
        self.assertEqual([], validator.new_duplicate_problems([], [tv, radio]))


class NetworkTests(unittest.TestCase):
    @mock.patch.object(validator, "_public_addresses", return_value=(True, None))
    @mock.patch.object(validator, "_safe_urlopen")
    def test_network_failure_is_returned_as_warning_text(self, safe_urlopen, _public):
        ...
        message = validator.check_url("https://media.example/live.m3u8")
        self.assertIn("request failed", message)

    @mock.patch.object(validator.socket, "getaddrinfo")
    def test_private_destination_is_not_requested(self, getaddrinfo):
        getaddrinfo.return_value = [(None, None, None, None, ("127.0.0.1", 0))]
        safe, reason = validator._public_addresses("internal.example")
